...
import { GOAL, DAILY_TARGET, WORKOUT_PLAN, MEAL_PLAN } from '@/lib/workout-data'
...
export default function DashboardClient() {
  ...
  const [todayNutrition, setTodayNutrition] = useState<{ calories: number; protein_g: number } | null>(null)
  ...
  useEffect(() => {
    async function load() {
      const [w, n, wk] = await Promise.all([
        ...
        supabase.from('nutrition_logs').select('calories,protein_g').eq('date', todayKey).maybeSingle(),
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Calo hôm nay', value: todayNutrition ? `${todayNutrition.calories}` : '—', unit: `/ ${DAILY_TARGET.calories} kcal`, color: todayNutrition && todayNutrition.calories <= DAILY_TARGET.calories ? '#22c55e' : '#f97316' },
          { label: 'Protein hôm nay', value: todayNutrition ? `${todayNutrition.protein_g}g` : '—', unit: `/ ${DAILY_TARGET.protein}g`, color: '#60a5fa' },
          { label: 'Cân hiện tại', value: `${currentWeight}kg`, unit: `mục tiêu ${GOAL.target_weight}kg`, color: 'var(--orange)' },
          { label: 'BF mục tiêu', value: `${GOAL.target_bodyfat_min}–${GOAL.target_bodyfat_max}%`, unit: 'body fat', color: '#a78bfa' },
        ].map(s => (
      ...
      {!todayWorkoutDay && (
        <div className="card" style={{ borderLeft: '3px solid #22c55e' }}>
          <div className="text-xs mb-1" style={{ color: '#22c55e' }}>HÔM NAY · CHỦ NHẬT</div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <div className="text-sm font-semibold" style={{ fontFamily: 'var(--font-display)' }}>Thực đơn hôm nay</div>
          <Link href="/nutrition" className="text-xs" style={{ color: 'var(--orange)' }}>Ghi log →</Link>
        </div>
        <div className="space-y-2">
          {MEAL_PLAN.map(m => (
            <div key={m.meal} className="flex items-center justify-between py-2 border-b" style={{ borderColor: 'var(--border)' }}>
              <div>
                <span className="text-xs font-semibold mr-2" style={{ color: 'var(--orange)' }}>{m.meal}</span>
                <span className="text-sm" style={{ color: 'var(--text-2)' }}>{m.items}</span>
              </div>
              <div className="text-right flex-shrink-0 ml-4">
                <span className="text-xs" style={{ color: 'var(--text-3)' }}>{m.calories} kcal</span>
                <span className="text-xs ml-2" style={{ color: '#60a5fa' }}>{m.protein}g P</span>
              </div>
            </div>
          ))}
