      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-6">
        {WORKOUT_PLAN.map(w => (
          <button key={w.day}
            ...
            style={{
              background: activeDay === w.day ? 'var(--orange)' : 'var(--surface)',
              color: activeDay === w.day ? 'white' : 'var(--text-2)',
              border: `1px solid ${activeDay === w.day ? 'var(--orange)' : 'var(--border)'}`,
              boxShadow: w.day === todayDay && activeDay !== w.day ? '0 0 0 2px rgba(249,115,22,0.3)' : 'none',
            }}>
            {w.day}
                ...
                <div key={ex.id} className="card card-hover"
                  ...
                  {isOpen && (
                    <div className="mt-4 space-y-4 animate-slide-up">
                      {/* Weight note */}
                      <div className="px-3 py-2 rounded-lg text-sm" style={{ background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.2)', color: 'var(--text-2)' }}>
                        <span className="font-semibold" style={{ color: 'var(--orange)' }}>📊 Khối lượng: </span>{ex.weight_note}
                      </div>

                      {/* Tips */}
                      <div>
                        <div className="text-xs font-semibold mb-2" style={{ color: '#22c55e' }}>✅ Tips</div>
                      ...
                      {/* Avoid */}
                      <div>
                        <div className="text-xs font-semibold mb-2" style={{ color: '#f87171' }}>⚠️ Tránh</div>
                        <ul className="space-y-1">
                          {ex.avoid.map((a, i) => (
                            <li key={i} className="text-sm flex gap-2" style={{ color: 'var(--text-2)' }}>
